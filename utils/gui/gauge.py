...
try:
    ...
    from PyQt5.QtCore import Qt, QTime, QTimer, QPoint, QPointF, SIGNAL, QRect, QSize
...
class AnalogGaugeWidget(QWidget):
    ...
    def rescale_method(self):
        ...
        self.change_value_needle_style([QPolygon([
            ...
            QPoint(int(-2), int(- self.widget_diameter / 2 * self.needle_scale_factor)),
    ...
    def create_scale_marker_values_text(self):
        ...
        for i in range(self.scala_main_count + 1):
            ...
            painter.drawText(int(text[0]), int(text[1]), int(text[2]), int(text[3]), int(text[4]), str(text[5])
)
